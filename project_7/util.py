...
class Counter(dict):
    ...
    def softmax(self):
        """
        Updates all values in the dictionary with the result
        of a safe (numerically stable) version of softmax.
        """
        maxValue = max(self.values())
        for key in self:
            self[key] = exp(self[key] - maxValue)
        self.normalize()
    ...
    def sortedKeys(self):
        ...
        sortedItems.sort(key=compare, reverse=True)
