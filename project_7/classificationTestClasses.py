...
def readFoodData(trainingSize=100, testSize=100):
    rootdata = 'pacmandata'
    rawTrainingData, trainingLabels = samples.loadPacmanData(rootdata + '/food_training.pkl', trainingSize)
    rawValidationData, validationLabels = samples.loadPacmanData(rootdata + '/food_validation.pkl', testSize)
    rawTestData, testLabels = samples.loadPacmanData(rootdata + '/food_test.pkl', testSize)
    trainingData = []
    validationData = []
    testData = []
    return (trainingData, trainingLabels, validationData, validationLabels, rawTrainingData, rawValidationData, testData, testLabels, rawTestData)
...
foodData = readFoodData(1000)

tinyContestData = readContestData(10, 10)
smallContestData = readContestData(50, 50)

bigContestData = readContestData(10000, 10000)
bigFoodData = readFoodData(10000, 10000)
bigSuicideData = readSuicideData(10000, 10000)
...
DATASETS = {
    ...
    "foodData": lambda: foodData,
    "tinyContestData": lambda: tinyContestData,
    "smallContestData": lambda: smallContestData,
    "bigContestData": lambda: bigContestData,
    "bigFoodData": lambda: bigFoodData,
    "bigSuicideData": lambda: bigSuicideData
}
...
DATASETS_LEGAL_LABELS = {
    ...
    "foodData": ["EAST", 'WEST', 'NORTH', 'SOUTH', 'STOP'],
    "tinyContestData": ["EAST", 'WEST', 'NORTH', 'SOUTH', 'STOP'],
    "smallContestData": ["EAST", 'WEST', 'NORTH', 'SOUTH', 'STOP'],
    "bigContestData": ["EAST", 'WEST', 'NORTH', 'SOUTH', 'STOP'],
    "bigFoodData": ["EAST", 'WEST', 'NORTH', 'SOUTH', 'STOP'],
    "bigSuicideData": ["EAST", 'WEST', 'NORTH', 'SOUTH', 'STOP']
}
...
def getInSampleAccuracy(data, classifier, featureFunction=dataClassifier.basicFeatureExtractorDigit):
    trainingData, trainingLabels, validationData, validationLabels, rawTrainingData, rawValidationData, testData, testLabels, rawTestData = data
    if featureFunction != dataClassifier.basicFeatureExtractorDigit:
        trainingData = list(map(featureFunction, rawTrainingData))
        validationData = list(map(featureFunction, rawValidationData))
    ...
    acc = 100.0 * correct / len(trainingLabels)
    serialized_guesses = ", ".join([str(guesses[i]) for i in range(len(trainingLabels))])
    print (str(correct), ("correct out of " + str(len(trainingLabels)) + " (%.1f%%).") % (acc))
    return acc, serialized_guesses
...
class GradeClassifierTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.accuracyScale = round(float(testDict['accuracyScale']), 1)
        ...
        self.evalInSample = testDict['evalInSample'].lower() == "true" if 'evalInSample' in testDict else None
        self.early_stopping = int(testDict['early_stopping']) if 'early_stopping' in testDict else None
        self.hidden_neurons = int(testDict['hidden_neurons']) if 'hidden_neurons' in testDict else None
        self.learning_rate = float(testDict['learning_rate']) if 'learning_rate' in testDict else None
    ...
    def grade_classifier(self, moduleDict):
        ...
        if self.max_iterations != None:
            if self.classifierModule == "neural_pacman":
        ...
        if self.evalInSample:
            return getInSampleAccuracy(data, classifier, featureFunction=featureFunction)
        else:
...
class MultipleChoiceTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        encryptedSolution = sha1(studentSolution.encode('utf-8').strip().lower()).hexdigest()
