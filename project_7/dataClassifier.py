...
def enhancedPacmanFeatures(state, action):
    ...
    Helpful hint: state.generateSuccessor(0, action) will compute what
    the GameState would be IF this action were taken.  You can then use
    the result to query any of the detailed information from GameState.
    (e.g. food, capsules, ghosts, walls, pacman, etc)
    """
    features = util.Counter()
    return features

def neuralFeatureExtractorPacman(state):
    """
    A neural feature extraction function.  Note that it constructs
    a different format of features from the perceptron extractors.

    You should return a util.Counter() of named features along with
    their values for the current state, along with a list of legal
    actions that can be selected from this state.

    IMPORTANT: You must always return all possible features that
               will ever be returned.  (You cannot omit certain
               features in some cases, but include them in others.)
    """
    features = util.Counter()

    # As an implementation detail, always provide a -1 bias feature.
    features['biasInput'] = -1

    # Pacman's current position?
    pacmanPosition = state.getPacmanPosition()

    # How much food is left?
    features['remainingFood'] = state.getNumFood()
    ...
    # remaining capsules 
    features['remainingCapsules'] = len(state.getCapsules())
    
    # closest ghost 
    ghostPos = state.getGhostPositions()
    closestGhost = float('inf') 
    for pos in ghostPos: 
        closestGhost = min(closestGhost, manhattanDistance(pacmanPosition, pos))
    ...
    for pos in foodPos: 
        closestFood = min(closestFood, manhattanDistance(pacmanPosition, pos))
    features['closestFood'] = closestFood * 100
    
    # number of ghosts
    features['numberGhosts'] = state.getNumAgents()
...
def readCommand( argv ):
    ...
    parser.add_option('-c', '--classifier', help=default('The type of classifier'), choices=['mostFrequent', 'nb', 'naiveBayes', 'neural', 'perceptron', 'scikit-neural', 'mira', 'minicontest'], default='mostFrequent')
    ...
    if(options.data=="digits"):
        ...
        printImage = None
        if (options.classifier == "neural" or options.classifier == "scikit-neural"):
            featureFunction = neuralFeatureExtractorPacman
    ...
    if(options.classifier == "mostFrequent"):
        ...
        # Implemented for pacman only.
        classifier = neural_pacman.NeuralClassifierPacman(legalLabels,options.iterations)
    elif(options.classifier == "scikit-neural"):
        # Implemented for pacman only.
        classifier = neural_pacman.ScikitNeuralClassifierPacman(legalLabels,options.iterations)
