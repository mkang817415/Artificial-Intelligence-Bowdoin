...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # get n alive ghost's most probable positions 
        most_likely_ghost_positions = [] 
        
        for distribution in livingGhostPositionDistributions:
            most_likely_ghost_positions += [distribution.argMax()]
            
        # get the closest ghost position from pacman 
        min_dist = float('inf')
        min_pos = "NONE"
        for ghost_pos in most_likely_ghost_positions: 
            real_dist = self.distancer.getDistance(pacmanPosition, ghost_pos)
            if real_dist < min_dist: 
                min_dist = real_dist
                min_pos = ghost_pos 
        
        # get the action that gets closest to the position 
        min_action_dist = float('inf') 
        min_action = "Stop"
        for action in legal: 
            next_state = Actions.getSuccessor(pacmanPosition, action)
            dist = self.distancer.getDistance(next_state, min_pos) 
